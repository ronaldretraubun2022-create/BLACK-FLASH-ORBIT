...
const readinessItems = [
  {
    ...
    description: "Responsive route workspace mounted inside the persistent layout.",
    icon: RadioTower,
  },
  {
    label: "Security Baseline",
    description: "Defensive access layer prepared for production integration.",
    icon: ShieldCheck,
  },
  {
    label: "Module State",
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mt-8">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {readinessItems.map(({ label, description: itemDescription, icon: ItemIcon }) => (
            <article
              className="rounded-2xl border border-white/10 bg-white/[0.035] p-5"
              key={label}
            >
              <ItemIcon className="text-cyan-300" size={19} />
              <h3 className="mt-5 text-base font-black text-white">{label}</h3>
